...
public class ListaDeEnterosEnlazada extends ListaDeEnteros {
	...
	public ListaDeEnterosEnlazada ordenar() {
		ListaDeEnterosEnlazada nue = new ListaDeEnterosEnlazada();
		int act, min = -1, mintot = 0;
		while (nue.tamanio()<this.tamanio()) {
			this.comenzar();
			min = this.proximo();
			while ((min<=mintot)&&!this.fin()) {
				min = this.proximo();
			}
			while (!this.fin()) {
				act = this.proximo();
				if ((act<min)&&(act>mintot)) {
					min = act;
				}
			}
			mintot = min;
			nue.agregarFinal(min);
		}
		return nue;
	}
	
	public ListaDeEnterosEnlazada combinarOrdenado(ListaDeEnterosEnlazada l2) {
		ListaDeEnterosEnlazada l1 = this.ordenar();
		ListaDeEnterosEnlazada r = new ListaDeEnterosEnlazada();
		l1.comenzar();
		l2.comenzar();
		Integer a=l1.proximo(),b=l2.proximo();
		while((!(l1.fin()))&&(!(l2.fin()))) {
			if (a<b) {
				r.agregarFinal(a);
				a = l1.proximo();
